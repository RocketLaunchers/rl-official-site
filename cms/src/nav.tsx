...
export const GUIDES: Guide[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      'Click Save.',
      'To put them on the team page, open Seasons → the current season → Roster, click “＋ Add member”, and pick the person.',
      'Give them a role; use “＋ Add another role” if they hold more than one (e.g. an officer who also leads a subteam).',
  ...
  },
  {
    ...
    steps: [
      ...
      'Use the ↑ / ↓ buttons on each role card to set its order (top = first). This is the order officers appear in on the team page, every season.',
      'Click Save, then Publish.',
    ],
  },
  {
  ...
  },
  {
    ...
    steps: [
      ...
      'Scroll to Roster. Find the member (or click “＋ Add member” and pick the person).',
      'Set their Role, and — for Lead / Co-Lead / Member — pick a Subteam. Officer roles use no subteam.',
      'To give one person more than one role, click “＋ Add another role” under them. Each role has its own “Show this role on the site” switch, so you can show one and hide another.',
      'You don’t set order numbers here — the team page orders people by role and subteam automatically. Click Save, then Publish.',
    ],
  },
  {
    id: 'ordering',
    title: 'How ordering, sorting & search work',
    tabs: ['roles', 'subteams', 'seasons', 'events'],
    steps: [
      'Three tools keep long lists manageable: Search (a box at the top of every list), Sort by (a menu that reorders the view), and ↑ / ↓ buttons (that set the real order shown on the site). There are no order-number boxes anymore.',
      'Lists you control the order of — Roles, Subteams, Rockets, Gallery albums — have ↑ / ↓ on each card. That order is what appears on the website.',
      'Roles: the ↑ / ↓ order is the master order for officers on the team page — set it once and every season matches. Subteams: their order sets how subteam leads and members line up.',
      'Team roster (Seasons → Roster): fully automatic. Officers sort by role order, then subteam leads and members by subteam — the same way every season. Nothing to set per person.',
      'Lists with no fixed order — People, Events, Constitution, Sponsors, News — have a “Sort by” menu (e.g. name, graduation year, date, category, status). This only changes what you see while editing; the site uses its own order (people where referenced, events/news/constitution by date).',
      'Cards start collapsed to save space — click a card (or its ▸) to open it, and “Collapse all / Expand all” to do the whole list at once. In Seasons, roster members, sponsors, and advisors each collapse this way; advisors also have their own “Sort by”.',
      'After any change, click Save, then Publish to make it live.',
    ],
  },
  {
    id: 'new-subteam',
