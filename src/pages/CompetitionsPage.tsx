  ...
  return (
    ...
    >
      {competitions.length === 0 ? (
        <p className="text-neutral-500 font-light">No competitions recorded yet.</p>
      ) : (
        <>
          {honors.length > 0 && (
            <section className="mb-16">
              <h2 className="flex items-center gap-2.5 text-neutral-300 text-[11px] uppercase tracking-[0.2em] font-light mb-6">
                <Icon name="trophy" className="w-4 h-4 text-cyan-300/80" />
                Awards &amp; Achievements
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {honors.map((h) => (
                  <div
                    key={h.key}
                    className="border border-cyan-400/20 bg-cyan-400/[0.03] p-5 flex items-start gap-3.5"
                  >
                    <Icon name="trophy" className="w-7 h-7 text-cyan-300/80 shrink-0 mt-0.5" />
                    <div>
                      <p className="text-white text-base font-light leading-snug">{h.label}</p>
