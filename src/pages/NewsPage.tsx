...
export default function NewsPage() {
  return (
    ...
    >
      {news.length === 0 ? (
        <p className="text-neutral-500 font-light">No posts yet.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {news.map((post) => (
            ...
            >
              {post.coverImage && (
                <div className="aspect-[16/9] bg-neutral-950 border-b border-white/10 overflow-hidden">
                  <img
                    src={post.coverImage}
                    alt={post.title}
                    loading="lazy"
              ...
              <div className="p-6 flex flex-col flex-1">
                ...
                <h3 className="font-display text-xl font-light text-white mb-3 tracking-tight group-hover:text-neutral-300 transition-colors">
                  {post.title}
                </h3>
                <p className="text-neutral-400 leading-relaxed font-light mb-4 text-[15px] flex-1">{post.excerpt}</p>
