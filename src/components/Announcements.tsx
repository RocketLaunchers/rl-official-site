...
function Cta({ label, href, large }: { label: string; href: string; large?: boolean }) {
  const cls = large
    ? 'inline-flex items-center gap-2 bg-accent text-on-solid px-6 py-3 text-[13px] tracking-[0.12em] font-medium hover:bg-accent hover:shadow-[0_0_22px_rgba(34,211,238,0.4)] transition-all duration-300'
    : 'inline-flex items-center gap-2 border border-accent/40 text-accent px-4 py-2 text-xs tracking-[0.12em] font-light hover:bg-accent/10 transition-all duration-300';
...
function Meta({ when, location }: { when?: string; location?: string }) {
  return (
    <div className="flex flex-wrap items-center gap-x-5 gap-y-1 text-sm text-ink-soft font-light">
      {when && (
  ...
  return (
    <div className={`bg-black/40 light:bg-well flex items-center justify-center overflow-hidden ${className ?? ''}`}>
      {src ? (
        ...
        <span className="text-accent/30 text-[11px] uppercase tracking-[0.2em]">Flyer</span>
      )}
    </div>
  );
}
...
function PrimaryAnnouncement({ event }: { event: EventItem }) {
  ...
  return (
    <div className="relative border border-accent/30 bg-gradient-to-br from-accent/[0.08] via-line/[0.02] to-transparent shadow-[0_0_60px_-18px_rgba(34,211,238,0.5)] overflow-hidden md:flex">
      <Flyer src={flyerSrc(event)} alt={event.title} className="md:w-2/5 min-h-[260px] border-b md:border-b-0 md:border-r border-accent/20 p-4" />
      <div className="p-7 md:p-10 md:w-3/5 flex flex-col gap-5">
        <h3 className="font-display text-3xl md:text-4xl font-light text-ink tracking-tight leading-tight">
          {event.title}
        </h3>
...
function CompactAnnouncement({ event }: { event: EventItem }) {
  const when = event.displayDate || event.date;
  return (
    <div className="border border-accent/20 bg-accent/[0.03] overflow-hidden flex flex-col sm:flex-row">
      <Flyer src={flyerSrc(event)} alt={event.title} className="sm:w-2/5 aspect-[4/3] sm:aspect-auto border-b sm:border-b-0 sm:border-r border-accent/15" />
      <div className="p-5 sm:w-3/5 flex flex-col gap-3">
        <h3 className="font-display text-xl font-light text-ink tracking-tight leading-tight">{event.title}</h3>
        <Meta when={when} location={event.location} />
        {event.description && (
    ...
    <section id="announcements" className="py-20">
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-accent text-xs uppercase tracking-[0.28em] font-light">Announcements</h2>
        </div>
